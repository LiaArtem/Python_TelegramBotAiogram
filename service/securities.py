    ...
    async def get_Read_ISIN_Securities(self):
        ...
        try:
            ...
            await db.execute("""CREATE TABLE IF NOT EXISTS SECUR_ISIN_PAY
                        ...
                        """)
            await db.execute("CREATE UNIQUE INDEX IF NOT EXISTS UK_SECUR_ISIN_PAY ON SECUR_ISIN_PAY (ISIN_SECUR_ID, "
                             "PAY_DATE, PAY_TYPE)")
            ...
            if self.securities_isin == "":
                ...
                if securities_name.find("%") >= 0:
                    cursor = await db.execute("SELECT * FROM SECUR_ISIN S WHERE S.CPDESCR like ? AND S.CURRENCY_CODE "
                                              "= ?", params)
                else:
