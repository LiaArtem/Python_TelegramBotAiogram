...
async def message_start(bot: Bot):
    await bot.send_message(chat_id=settings.bots.TELEGRAM_ADMIN_CHAT_ID, text='Message START!')


# отправление сообщения ежедневно
async def message_day(bot: Bot):
    await bot.send_message(chat_id=settings.bots.TELEGRAM_ADMIN_CHAT_ID, text='Message DAY!')
